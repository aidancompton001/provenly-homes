...
function CellIcon({ value }: { value: string }) {
  ...
  if (value === "?") {
    return (
      <span className="font-body text-charcoal-light text-lg" aria-label="Unsicher">
        ?
      </span>
    );
  }
  return <span className="font-body text-sm text-charcoal-light">{value}</span>;
}
      ...
      <Container>
                    ...
                    <th
                      ...
                      className={[
                        ...
                        i === comparison.columns.length - 1
                          ? "border-l-2 border-copper bg-copper/5"
                          : "",
                      ].join(" ")}
                    >
                      {col}
              ...
              <tbody>
                {comparison.rows.map((row) => (
                  <tr key={row.category} className="border-t border-sand">
                    ...
                    <td className="py-4 px-4 text-center border-l-2 border-copper bg-copper/5">
        ...
        >
          {comparison.rows.map((row) => (
            <div
              key={row.category}
              className="bg-white rounded-xl p-4 shadow-[0_2px_8px_rgba(58,58,58,0.06)] border border-sand"
            >
              <p className="font-heading text-base font-semibold text-charcoal mb-3">
                {row.category}
              </p>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  <p className="font-body text-xs text-charcoal-light mb-1">
                ...
                <div className="bg-copper/5 rounded-lg p-1">
                  <p className="font-body text-xs text-copper font-medium mb-1">
